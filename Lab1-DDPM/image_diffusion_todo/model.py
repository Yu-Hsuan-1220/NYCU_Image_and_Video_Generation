...
class DiffusionModule(nn.Module):
    ...
    def get_loss_mean(self, x0, class_label=None, noise=None):
        ...
        # Closed-form posterior mean \tilde{mu}_t(x_t, x_0). This uses the true
        # x0, not a clamped prediction, so it is the exact regression target.
        sched = self.var_scheduler
        beta_t = extract(sched.betas, t, x0)
        alpha_t = extract(sched.alphas, t, x0)
        alpha_bar_t = extract(sched.alphas_cumprod, t, x0)
        # alpha_bar_{t-1} is 1 at t = 0, matching DDPMScheduler.step.
        is_first = (t == 0).reshape(-1, *([1] * (x0.ndim - 1)))
        alpha_bar_t_prev = extract(sched.alphas_cumprod, (t - 1).clamp(min=0), x0)
